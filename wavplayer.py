...
#
# MicroPython class that plays a WAV file over I2S, non-blocking (IRQ-driven refill), so
# playback continues in the background after play() returns.
#
# Pulled in via raptor09010's Micropython-ES8311-Library
# (github.com/raptor09010/Micropython-ES8311-Library, MIT), which bundles this unmodified
# from Mike Teachman's micropython-i2s-examples project. Used as-is here, no mck_pin support
# added -- this firmware's machine.I2S (ports/esp32/machine_i2s.c, specific to this ESP32
# port, NOT the generic extmod/machine_i2s.c other MicroPython ports use) has no mck=
# parameter at all, so the ES8311's MCLK line is generated separately, via machine.PWM in
# es8311.py, same as the original library did it. See music_player.py for wiring/usage.
#
# Example:
...
#
# All methods are non-blocking. The WAV file header is parsed in the play() method to get
# audio parameters.
